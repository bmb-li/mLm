...
import { loadLlamaModelInfo } from '../../modules/llama.rn/src' // import 'llama.rn'
import { getAllExampleModelDefinitions } from '../utils/exampleModels'
import { useStoredCustomModels } from '../hooks/useStoredSetting'
import {
  buildCustomModelFiles,
  buildMultimodalModelFiles,
  buildRegularModelFiles,
  buildTTSModelFiles,
...
const MODEL_INFO_MODELS = getAllExampleModelDefinitions('See')

export default function ModelInfoScreen() {
  const { theme } = useTheme()
  const themedStyles = createThemedStyles(theme.colors)
  const { styles, additionalStyles } = createStyles(theme, themedStyles)
  ...
  const renderSingleModelInfo = (fileInfo: ModelFileInfo) => {
    ...
    return Object.entries(info).map(([key, value]) => {
      const displayValue = formatModelInfoValue(value)
          ...
          <View key={fileInfo.path} style={additionalStyles.modelFileContainer}>
            <View
              style={[
                ...
                additionalStyles.fileHeaderContainer,
              ]}
            >
              <Text style={additionalStyles.fileHeaderText}>
                {fileInfo.name}
              </Text>
            </View>
            <ScrollView
  ...
  return (
    <View style={styles.container}>
      ...
      <ExampleModelSetup
        description='Download and explore detailed information about available AI models. Tap "See" on any model to view its metadata and specifications.'
        defaultModels={MODEL_INFO_MODELS}
        customModels={customModels || []}
        onInitializeCustomModel={async (customModel, modelPath, mmprojPath) => {
          await loadModelInfo(
            `${customModel.id} (${customModel.quantization})`,
            buildCustomModelFiles(customModel, modelPath, mmprojPath),
          )
        }}
        onInitializeModel={async (model, ...paths) => {
          if (model.kind === 'multimodal') {
            await loadModelInfo(
              model.title,
              buildMultimodalModelFiles(paths[0] || '', paths[1] || ''),
            )
            return
          }
          ...
          if (model.kind === 'tts') {
            await loadModelInfo(
              model.title,
              buildTTSModelFiles(paths[0] || '', paths[1] || ''),
            )
            return
          }
          ...
          await loadModelInfo(
            model.title,
            buildRegularModelFiles(paths[0] || ''),
          )
        }}
        ...
        progressText="Loading model information..."
